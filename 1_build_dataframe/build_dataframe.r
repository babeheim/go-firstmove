...
gogod[gogod[, "filename"] == "./temp/1960-02-03b.sgf", "RE"] <- "V" 
# "087-567214713", filed under "1960-69/1960-02-03b.sgf"
...
drop <- which(bio.data$Birth.Year == "??" | bio.data$Player.Name == "Kikuchi Yasuro")
...
gogod$BA <- as.numeric(substr(gogod$DT, 1, 4)) -
  bio.data$Birth.Year[match(gogod$PB, bio.data$Player.Name)]
...
for (i in 1:n.games) {
  ...
  use.win.cfrq.1p[i] <- mean(black.won[valid.range & fourfour]) -
    mean(black.won[valid.range & !fourfour])
  # population use win is relative to Threefour, not to average
  # does this difference matter? I'll have to change this and find out...
  ...
  b.win.cfrq.1p[i] <- mean(black.won[focal.as.black & valid.range]) -
    mean(black.won[valid.range])
  b.use.win.cfrq.1p[i] <- mean(black.won[focal.as.black & fourfour & valid.range]) -
    mean(black.won[focal.as.black & valid.range])
...
write.csv(regression.table, "./temp/fourfour_regression_table_24m.csv",
